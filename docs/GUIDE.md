...
centerline in 112.520 s and the generated racing line in 108.096 s — a 4.424 s gain from geometry
...
the `f1_2026` reference car laps the 3D Catalunya in 112.520 s on the centerline and 108.096 s on
...
- **The T2 driver is stable, not at the limit.** The transient tier's ideal driver tracks a
  **corner-scaled** speed reference — the full QSS profile on the straights, an ~0.85 stability
  margin where the profile rides the lateral grip limit, with ellipse-aware braking and traction
  feasibility shaping the transitions — so a T2 lap runs ~14–17% slower than T0/T1 while its
  *top speeds* now come within a few percent of the QSS profile. Pushed to the raw profile it
  still spins: the margin at the limit is the honest boundary of this driver. The T2 *physics*
  is validated (its operating points stay inside the T1 grip envelope); the remaining pace gap
  is driver competence at the limit, and every T2 result records the margin (Chapters 8, 13).
...
- **Temperature and wear** are live (v0.3): the `.tyr` `thermal:` and `wear:` blocks drive a physically-based thermal ring (surface/carcass/gas nodes, a grip window in temperature) and an Archard wear + cliff model, both marched as slow states in T0/T1 and T2. Grip depends on temperature, tires warm up and go off, and a *stint* — not just a lap — is simulable. The parameters are inverse-calibrated from stint pace by `outlap.wearcal`. Opt in with `tire_thermal=True` (default on for stints); the theory is in [`docs/theory/tire-thermal.md`](theory/tire-thermal.md) + [`tire-wear.md`](theory/tire-wear.md).
...
- An ERS (energy-recovery system, e.g. an F1 MGU-K) is reduced to a **power cap**: peak deployment power times a speed-dependent taper curve, then capped by the machine's own torque envelope *at the engaged gear's shaft speed*, $P \le \tau(\omega_\text{crank})\,\omega_\text{crank}$, and converted to force as $\eta P / \max(v, 1.0)$. A machine that shares a drivetrain node with the engine (an F1 MGU-K on the crank) turns at whatever speed the engine's engaged gear dictates, so below its base speed it is **torque**-limited: its wheel force is the flat $\tau\cdot\text{ratio}\cdot\eta / r$, not a $1/v$ curve. (The 1 m/s floor in the denominator is therefore vestigial for a crank-mounted machine — the two speed factors cancel — and still guards a machine with no shared node.) Per-lap deployment budgets and override modes are *not* enforced at T0 — a permanent note in every result says so.
...
The driver's one deliberate idiosyncrasy is the headline caveat of §1.5: it tracks a **corner-scaled stability margin**. The speed reference is shaped per station from the raw QSS profile (`outlap_qss::margin`): full profile speed where lateral demand is low, ~0.85 of it where the profile rides the lateral grip limit, each corner's margin propagated back through its braking zone plus a ~1.5 s settle ramp, and both transitions kept dynamically feasible by friction-ellipse-aware braking and traction passes evaluated at the path's true `g_normal`. Two driver-side stabilisers complete the picture: a **sideslip damper** (`δ −= k_β·β`, the correction for a translational "crab" slide the yaw damper cannot see) and a **wheel-slip pedal governor** (the ideal driver modulates throttle against drive wheelspin — with race gearing, low-gear torque is a multiple of the grip limit). The margin at the limit is not laziness — pushed to the raw profile, the closed loop *spins the car*, which is Chapter 2's "the QSS envelope is not filtered for stability" caveat made concrete. The result: top speeds within a few percent of the QSS profile, a lap ~14–17% slower overall, honest about why in its provenance (`speed_margin` attr) and in `docs/validation/limebeer.md`'s decomposition.
...
the closed-loop time integration: the ideal driver tracks the **corner-scaled** reference of
§8.7.2 — the full T0 profile where lateral demand is low, `speed_margin` × it at the lateral
grip limit (default 0.85; the resolved value is recorded in the result) —
...
- **The lap-time gate is recorded, not enforced.** The honest ≤ 1 % lap-time ambition was measured against the transient tier and the time-weighted line when they landed: even together they do not close it — the T2 lap on this geometry is ~+28% over the optimal-control oracle, dominated by the driver's corner stability margin (~+14% of T0) on top of the geometry (~5 pp), the ~2.2 s quasi-steady-versus-optimal-control floor the paper itself cites, and ~1.5 s of envelope conservatism. The full decomposition, and the wide tripwire that keeps the recorded band from drifting silently, live in `docs/validation/limebeer.md`.
...
- **The QSS↔transient parity gate is split into what holds and what is recorded.** The *asserted* gate is **hull containment**: every `(a_x, a_y)` operating point of a T2 closed-loop lap must stay inside the T1 g-g-g-v envelope (≤ 2 % exceedance allowed; measured **0.0 %** on all three reference cars, in both the Rust harness and `python/tests/test_parity.py`) — the transient physics never produces grip the quasi-steady physics says it cannot have. The lap-time/apex parity figures (0.3 % / 1 %) are *recorded, not asserted*: the T2 driver's corner stability margin puts the lap ~+14–17 % from T0 (top speeds within a few percent), a driver-competence gap at the limit, not a physics error. A time-indexed golden transient lap pins the whole T2 trajectory against silent drift, the T2 step is zero-allocation (dhat-gated), and its throughput is recorded with a regression tripwire.
...
- **The T2 driver is stable, not at the limit.** The transient tier integrates the full 7-DOF closed loop (Chapter 8, §8.7) behind a corner-scaled stability margin: full QSS speed on the straights, ~0.85 of it at the lateral grip limit, wheelspin governed by the driver's pedal. Pushed to the raw profile it spins. A T2 lap is therefore ~14–17 % slower than T0/T1 — almost entirely in the corners — while its straight-line speeds match the profile to a few percent. The physics parity is *gated* (hull containment, Chapter 13); the pace parity is *recorded*. Use T2 for the time-domain story — traces, shifts, transient loads — not as a faster lap-time oracle. An at-the-limit driver is the tier's next step.
...
- **Tire thermal + wear are live (v0.3), opt-in per solve.** With `tire_thermal=True` the tires heat (surface/carcass/gas nodes), gain and lose grip through a temperature window, wear (Archard), and fall off a cliff — marched as slow states in every tier and calibrated by `outlap.wearcal`. With it off (the single-lap default) the tires use a fixed friction coefficient at reference pressure and camber, so frozen-tire laps stay byte-identical. (The *machine* thermal network of Chapter 9 is a separate model for the electric drive unit.)
...
- **Multi-lap stints are live (v0.3); fuel mass is not.** `solve_stint`/`solve_transient_stint` (and `solve_stint_dataset`) run a stint carrying the tire thermal + wear slow state lap-to-lap. Fuel-burn mass loss is still a future slow state, and batch sweeps + a CLI are the road-to-1.0 items below.
...
Two honest scope notes are part of the v0.2.5 record, both decomposed in `docs/validation/limebeer.md` rather than hidden: the ≤ 1 % Limebeer lap-time ambition was measured and **not** achieved (the T2 driver's corner stability margin dominates — a driver-competence gap at the limit, not a physics error), and the transient step's original throughput target proved unreachable at full Magic-Formula fidelity (the measured number is recorded and tripwired instead). Ambitions that miss are recorded with their reasons — that is the project's validation culture (Chapter 13) applied to itself.
...
- **Speed margin (T2)** — the stability margin the transient driver keeps at the lateral grip limit (default 0.85), applied corner-scaled: full profile speed on the straights, the margin where grip is fully used, ellipse-feasible transitions; recorded on every T2 result (Chapter 8).
