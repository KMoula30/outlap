...
- **No transient tiers yet.** Requesting `tier="t2"` or `"t3"` raises a typed "not implemented"
  error; `crates/outlap-transient` is a placeholder (T2 arrives in M4, T3 in M6). The QSS-versus-
  transient parity gates are therefore a committed future check, not a current one.
...
- Requesting `t2` or `t3` fails loudly with a typed "not implemented yet (arrives in M4/M6)" error rather than silently downgrading.
